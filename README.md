...
- **Connecting an existing join-model row does not work yet** ([#41](https://github.com/flolefebvre/prisma-factorio/issues/41)). A model whose only unique constraint is compound is matched on Prisma's generated compound selector — `{ userId_teamId: { … } }` — which a flat row of scalars does not satisfy, so every route a row reaches a relation field by fails alike: `has([membership])`, a row or a list of them standing in the field itself, and a row drawn from a [recycle](#recycle) pool. Pass native relation input meanwhile.
