...
// A constraint over one field is no compound constraint — Prisma exposes it as the flat scalar the
// splat already carries — and an unnamed one is exposed under its fields joined with `_`.
function asCompoundKeys(constraint: { name: string | null; fields: readonly string[] } | null): CompoundKey[] {
  if (constraint === null || constraint.fields.length < 2) return [];

  return [{ name: constraint.name ?? constraint.fields.join("_"), fields: [...constraint.fields] }];
}

// Prisma's own parser reads the text, so the answer is exact for whatever the schema declares. A
// text it refuses — `getDMMF` hands back an error value rather than throwing — yields no constraint
// at all, for any model, leaving the flat scalars as they always were: refusal is of the whole
// schema, which a generated client never carries invalid, but a parser older than the schema's own
// syntax would.
function parsedCompoundKeys(schema: string): Map<string, CompoundKey[]> {
  const dmmf = getDMMF({ datamodel: schema });
  const keys = new Map<string, CompoundKey[]>();

  if ("type" in dmmf) return keys;

  for (const model of dmmf.datamodel.models) {
    const constraints = [
      ...asCompoundKeys(model.primaryKey),
      ...model.uniqueIndexes.flatMap((index) => asCompoundKeys(index)),
    ];

    if (constraints.length > 0) keys.set(model.name, constraints);
  }
...
function heldCompoundKeys(schema: string): Map<string, CompoundKey[]> {
  const held = compoundKeys.get(schema) ?? parsedCompoundKeys(schema);
